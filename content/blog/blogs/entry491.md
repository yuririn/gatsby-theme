...
description: 先日 スプレッドシートをJSONデータ化し、React 側で受け取ってページを量産しました。axios、react-router-domなどの使い方をまとめました。
lead: ["先日 スプレッドシートをJSONデータ化し、React 側で受け取ってページを量産しました。axios、react-router-domなどの使い方をまとめました。"]
---
## 前提条件
前提条件は以下です。
...
|-|-|-|-|
|1|アイテム01|https://placehold.jp/00ff00/0000ff/150x150.png|アイテム01の情報|
...
## タイトルタグなどをreact-helmetで設定する
*react-helmet*を使ってSEO詳細情報を追加します。
...
```
同様にList.jsにも*Helmet*を追加したらヘッダー内のåタイトルや説明を自由に編集できるようになります。
