...
|*フレージングコンテンツ*|`<a>`, `<abbr>`, `<area>` （map要素内にある場合）<br/> `<audio>`, `<b>`, `<bdi>`, `<bdo>`,<br/>`<button>`,`<br/>` `<canvas>`, `<cite>`, `<code>`, `<data>`,`<br/>`<br/>	`<datalist>`, `<del>`, `<dfn>`, `<em>`, `<embed>`,<br/>`<i>`, `<iframe>`, `<img>`, `<input>`,<br/>`<ins>`, `<kbd>`,<br/>`<keygen>`, `<label>`, `<map>`, `<mark>`, `<meter>`,<br/> `<noscript>`, `<object>`,<br/>`<output>`, `<picture>`,`<br/>` <br/>`<progress>`, `<q>`, `<ruby>`, `<s>`,<br/> `<samp>`, `<script>`, `<select>`,<br/>`<small>`,`<br/>` `<span>`,<br/>`<strong>`, `<sub>`, `<sup>`,<br/> `<template>`, `<textarea>`, `<time>`,<br/> `<u>`, `<var>`,<br/> `<video>`, `<wbr>`, テキスト||
|*エンベディッドコンテンツ*|`<audio>`, `<canvas>`, `<embed>`, `<iframe>`, `<img>`, `<object>`, `<picture>`, `<video>`||
|*インタラクティブコンテンツ*|`<a>` （HTML 5.1ではhref属性がある場合に限る）,<br/> `<audio>` （controls属性がある場合）,<br/> `<button>`, `<details>`, `<embed>`, `<iframe>`,<br/> `<img>` （usemap属性がある場合）,<br/> `<input>`（type="hidden"の場合は除く）,<br/> `<<keygen>`, `<label>`,`<br/><object>`<br/>（usemap属性がある場合）（HTML 5.1では除外）,<br/> `<select>`, `<textarea>`,<br/> `<video>`（controls属性がある場合）||
