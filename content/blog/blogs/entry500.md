...
私のブログはGatsbyJSでできているのでローカル側のGraphQLで記事のみ抽出してシートを作りました。やり方は、、、マニアックすぎるので割愛します。
...
#### ディメンションション設定をしてデータがきちんと取り込めているか確認する

URLとタイトルを表示してみます。

ディメンションにURLとTITLE（先頭行の名称）を選びます。以下のように表示されていれば成功です。

![ディメンションション設定](./images/2022/05/entry500-8.png)

### Search Console と連携する
次に、Search Consoleのデータを取り込みます。
<br>先にデータの追加を行います。対象のドメイン、URLのインプレッション、Webを選びます。<br>スクショでは切れてますが、右下に追加ボタンがあるのでクリック。
...
結合の設定（Confiture join）をclick。URLとLanding Pageの一致を条件とし、左外部結合を選びます。
![Confiture joinをclick。URLとLanding Pageの一致を条件とし、左外部結合を選びます。](./images/2022/05/entry500-12.png)
...
![指標はImpressions（表示回数）・URL Clicks(クリック数)・URL CTL（クリック率）・Avarage Position（掲載平均順位）](./images/2022/05/entry500-11.png)


URLがどれだけの指標があるのか分かるので各ページの価値が分かります。

![空のレポートを作成しておく](./images/2022/05/entry500-14.png)

### Google アナリティクス と連携する（今の所ユニバーサルアナリティクス）
...
```
そこで*CONCAT関数*を使ってフィールドを作成し一致するURLを作成します。
...
さきほどは取得するデータをセットしただけなので、実際に出力する表に必要なデータを設定していきます。
...
あとはシートにフィルターを掛けて、リライト対象記事や*noindex(※)*をセットするだけです。

例えば、以下のような対応を考えることができます。

|Impressions|URL CTL|Avarage Position|PV数|対応|
|-|-|-|-|-|
|1ヶ月500以上|3%以下|20位以内|-|タイトルやディスクリプションを中心にリライト|
|1ヶ月10以下|0|70以上|null|noindexを追加|

※ここで言う noindex とはGoogle によるページのインデックス登録をブロックでき、検索結果にページを表示させなくしてしまうことです。
```HTML
<meta name="robots" content="noindex" >
```
サイトの成長過程で対策は変わってくると思いますのであくまで参考にしてみてください。

## まとめ・データポータルを使って各種データをまとめると全貌が把握できる
データポータルでデータを統合すると、全体が把握しやすくなり、総合的に見てどの記事を分析すればいいかなど把握しやすくなります。

今回この表を作るのも結構手探りで作りましたが、まだまだ紹介できていない機能があるのでまた記事にまとめられたらなーと想っています。
