...
description: レンジスライダー（つまみのある範囲の選べるフォーム）のJSライブラリご紹介。HTMLだと、UIはブラウザでマチマチだし値は1パーツに一個しかハンドル（つまみ）がなく、使い勝手が悪い。基本の実装方法、オプションの詳細、動的レンダリング、オプションのアップデート方法等詳しく解説します。
lead: ["HTMLだと、ブラウザで見た目がマチマチだし値は1パーツに一個しか付与できず、使い勝手が悪い。レンジスライダー（つまみのある範囲の選べるフォーム）のJSライブラリ・noUiSliderの使い方についてメモしました。導入の仕方やレンジスライダーを使った動的フォームの実装方法などのコードサンプルを中心に詳しく解説します。"]
---
## この記事の対象者
