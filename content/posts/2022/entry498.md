---
title: パンくずリストの構造化データ（jsonLD）の書き方【2026年盤】
date: 2022-04-21
...
faq:
  - q: "パンくずリストの構造化データを設置したのに、検索結果でディレクトリ名（アルファベット）のまま表示されるのはなぜ？"
    a: "Googleがページ上のテキストよりもURL構造を優先して読み取っている可能性があります。解決策として、**JSON-LD内の `name` プロパティに、日本語のカテゴリー名や記事タイトルを明示的に記述**してください。また、JSON-LD内のURLは、必ず [絶対パス（https://...）](#構造化データを付与) で記述するのが鉄則です。"
  - q: "構造化データエラー「項目 'item' がありません」と表示されます。"
    a: "パンくずリストの最後の項目（現在地）にURL（item）を記述していない場合、Googleのテストツールで警告が出ることがあります。現在地にはリンクを貼らないのが一般的ですが、構造化データ上は **`@id` に現在のURLを記述** しておくことで、エラーを回避しつつ [詳細度の高いリッチリザルト](#seoで効果大パンくずリストの詳細度を上げる方法) を狙うことができます。"
  - q: "JSON-LD と Microdata、SEOに有利なのはどちらですか？"
    a: "Googleは **JSON-LD を公式に推奨** しています。HTMLタグを汚さず、メンテナンス性も高いため、今から実装するなら JSON-LD 一択です。当ブログでも推奨している [JSON-LD のテンプレート](#構造化データを付与) をコピペして、ご自身のサイトURLに書き換えてみてください。"
  - q: "スマホでパンくずリストが長すぎて崩れてしまいます。"
    a: "パンくずを省略（...）してしまうとSEO効果が薄れるため、**横スクロール（overflow-x: auto）** での実装が現在のトレンドです。ユーザーの利便性を損なわずに全階層を見せられる [スクロール対応のサンプルコード](#長くなるとスクロールするパンくずリスト) を紹介しています。"
---
