...
faq:
  - q: "GCashは日本国内（フィリピン国外）でも使うことができますか？"
    a: "はい、可能です。Alipay+（アリペイプラス）と提携している店舗であれば、日本でもGCashのQRコード決済が利用できます。また、[GCash Card](#gcash-card-を作ると-atm-から-現金が引き出せる) を持っていれば、海外のATM（マスターカード対応）から現地通貨を引き出すこともできますが、為替レートや手数料に注意が必要です。"
  - q: "観光客（短期滞在者）でもGCashのアカウント作成や本人確認（Fully Verified）はできますか？"
    a: "フィリピン現地の電話番号（SIMカード）があれば作成可能です。本人確認にはパスポートが利用できますが、登録住所をフィリピン国内にする必要があるため、ホテルの住所等で通るか確認が必要です。長期滞在でない場合は、現金やクレジットカードとの併用をおすすめします。"
  - q: "GCash Card でATMから現金を引き出す際、どの銀行がおすすめですか？"
    a: "銀行やATMの種類によって手数料が異なります。セブンイレブンのATMは [手数料18ペソ（2023年時点）](#gcash-card-で実際にお金を引き出してみる) と比較的安価で、操作も分かりやすいためおすすめです。フィリピンの銀行ATMでは1回あたり10,000〜20,000ペソが上限となることが多いです。"
  - q: "GCash Card が届かない、または紛失した場合はどうすればいいですか？"
    a: "配送が遅れるのはフィリピンでは「あるある」ですが、2週間以上届かない場合はヘルプセンターへ問い合わせましょう。紛失時は、アプリの [My Linked Accounts](#gcash-card-をアクティベートする) から即座にカードを「Lock（ロック）」できるので、不正利用を確実に防げます。"
---
