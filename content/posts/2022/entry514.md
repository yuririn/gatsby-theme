...
faq:
  - q: "Swiperをスマホだけで有効にし、PCでは普通のリスト表示（横並び）にするには？"
    a: "ブレークポイントを判定して `new Swiper` と `destroy()` を切り替えるJSの実装が必要です。CSS側でもPC時に `display: flex;` などでスライダー特有のスタイルを打ち消す必要があります。具体的なコードは [Swiperでスマホ、PCで切り替える方法](#swiper-でスマホ-pcで切り替える方法-単体-) セクションで解説しています。"
  - q: "同じページに複数のSwiperがある場合、destroy() がエラーになります。"
    a: "全てのSwiperに同じクラス名（.swiper）で指定していると、破棄する際に対象が特定できずエラーになることがあります。各要素に **IDやユニークなクラスを付与** し、配列で管理するのが正解です。[複数のSwiperを切り替える方法](#複数のswiperをスマホ-pcで切り替える方法) のセクションを参考にしてください。"
  ...
  - q: "Swiper v11 を使っていますが、古い記事のコードが動きません。"
    a: "SwiperはバージョンアップによりCDNのパスや記述方法（ES Modules形式など）が頻繁に変更されます。2025年現在の [最新の読み込み方法](#swiper-基本の使い方) に書き換えることで解決します。"
---
