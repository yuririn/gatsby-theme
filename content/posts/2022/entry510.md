...
faq:
  - q: "Microsoft Clarity を導入するとサイトの表示速度は遅くなりますか？"
    a: "Clarity のスクリプトは非同期で読み込まれるため、Webサイトの表示速度（コアウェブバイタル）への影響は最小限に抑えられています。また、無料で全機能が使えるため、有料ツールからの乗り換え先としても優秀です。導入は [Googleタグマネージャー連携](#clarityのインストール) を使うのが最も簡単で安全です。"
  - q: "ヒートマップが「We couldn't find any clicks.」と表示され、見られない時の対処法は？"
    a: "URLのフィルタリング設定が原因の可能性があります。デフォルトの正規表現による抽出だとデータがうまく合算されないことがあるため、[Pathフィルターで完全一致のURLを指定](#path-フィルターでの絞り込み方法) してみてください。記事内の [ヒートマップが表示されない場合の対処法](#ヒートマップ表示されない場合２０２２年１２月追記) で詳しく手順を解説しています。"
  - q: "Clarity で特定のユーザーの個人情報や入力内容が録画されるのが心配です。"
    a: "デフォルトで「Masking（マスキング）」機能が有効になっており、パスワードや機密情報は自動で隠されます。さらに安心を高めたい場合は、設定画面から [Masking モードを 'Strict'（すべて隠す）](#maskingマスキング-のセットアップ) に変更するか、特定のHTML要素を個別に指定して隠すことも可能です。"
  - q: "GA4（Google アナリティクス 4）と連携して何ができますか？"
    a: "GA4と連携すると、GA4の管理画面上で「特定の行動をしたユーザーのClarity録画データ」を確認できるようになります。数値だけでは分からない「なぜ離脱したのか？」を動画で補完できるため、[Google アナリティクス連携](#google-アナリティクスと連携) は必須の設定と言えます。"
---
...
![絞られた状態](./images/06/entry510-14.png)
...
![絞られた状態](./images/06/entry510-18.png)
...
![スクロール量](./images/06/entry510-24.png)
