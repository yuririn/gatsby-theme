...
faq:
  - q: "Microsoft Clarity を導入するとサイトの表示速度は遅くなりますか？"
    a: "Clarity のスクリプトは非同期で読み込まれるため、Webサイトの表示速度（コアウェブバイタル）への影響は最小限に抑えられています。また、完全無料で全機能が使えるため、有料ツールからの乗り換え先としても優秀です。導入は [Googleタグマネージャー連携](#clarityのインストール) を使うのが最も簡単で安全です。"
  - q: "ヒートマップが「We couldn't find any clicks.」と表示され、見られない時の対処法は？"
    a: "URLのフィルタリング設定が原因の可能性があります。デフォルトの正規表現による抽出だとデータがうまく合算されないことがあるため、[Pathフィルターで完全一致のURLを指定](#path-フィルターでの絞り込み方法) してみてください。記事内の [ヒートマップが表示されない場合の対処法](#ヒートマップ表示されない場合2022年12月追記) で解説しています。"
