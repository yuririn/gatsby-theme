...
Slackは適度にメッセージを装飾しながら書くことができます。
...
|*引用*|誰かのメッセージなどを引用したいとき|ctrl/cmd + shift + 9|先頭に>（大なり）+ スペース|
...
自分には大切だけど、他人には関係ないメッセージはブックマークしておきます。
...
私は現在Slackで9ワークスペースぐらいあります。主要なワークスペースはテーマを変えるようにしています。<br><br>

理由は、**誤爆防止（間違えてメッセージを送らないようにするため）**です。<br><br>

昔全然違うワークスペースへメッセージして*寿命が縮む思い*をしたことがあります。
...
以下画像の「メンバーがこう言ったら」の部分は*,（カンマ）*区切りで複数設定可能です。<br>botのメッセージを改行したいときは\n（バックスラッシュ + n、Windowsは¥（円）記号）を使います。
...
<div class="box">
  ...
  中にはメッセージを削除して証拠隠滅（いんめつ）する人もいます。<br>防止するために、編集権限に期間を設けることをオススメします。<br>
...
メッセージ入力欄にコマンドを打ち込めば、リマインダー登録できます。

<small>※ コマンドはメッセージ欄に入力して実行します。</small>
