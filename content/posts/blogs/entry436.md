...
なので人間、洋服サイズ感に例えてみます。*人間がHTML要素*だと思ってください。
...
HTMLでは要素と要素の距離を`margin`と言います。

marginのおもしろいところは縦方向の余白は少ない方の値が相殺されます。
これを折りたたみマージンと言います。ちなみに横方向では相殺されません。

ちなみにコロナ禍のフィリピンでは人と人との`margin`は2メートルとやや広めですが、誰一人守ってないです。
折りたたまれてますね。
...
今までの`margin`や`padding`は物理プロパティといって要素がどう並ぼうが上下左右に値を設定できました。
...
```
物理プロパティでは左など位置を指定したら必ずその位置に余白を指定できます。
...
文字方向が英語と同じの場合は通常通り上が先頭、下が末尾になります。日本語と同じ場合は右が先頭で左が末尾です。

![インラインの軸・文字の並ぶ方向](./images/2021/01/entry436-01.jpg)
...
```
文字の向き（縦書き横書き等）に左右されるので余白の位置が左右・上下が変わります。
...
```
## まとめ・多言語サイトでは役に立ちそう！
前から気になっていたので今回は理論プロパティ・margi-blockとmargin-inlineについて掘り下げてみました。
