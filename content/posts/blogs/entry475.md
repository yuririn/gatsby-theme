...
JavaScriptが分かって、Googleアカウントさえあればこのお悩みを解決できます。

今回はGoogle Apps Script（GAS）を使って、Paypalの請求書などの**出力したなどをPDFやPNGファイルを整理する方法**を具体的に紹介していきます。
...
4. [Googleドライブでファイルを保存しリネーム](/blogs/entry476)
5. [スプシにファイル情報を書き出す](/blogs/entry476)
6. [メニュー操作で保存処理を常に実行できるようにする](/blogs/entry476)

## スプシを準備。データシートに案件名、顧客名、ステータスを登録しておく
まずは1こ、スプシを準備します。スプシ名は今回*請求書管理*とかにしておきましょう。

今回は案件ごとに管理するため、*請求書*と*データ*の2つのシートを用意します。

データシート案件名、顧客名、ステータスを登録しておきます。本来はシートを分けたほうがいいですが、今回「請求書ファイルをGoogleドライブに保存する方法のご紹介」なので、便宜上まとめています。
...
一行目にid、案件名、顧客名、ステータス、請求日、支払日を見出しとして登録しておきます。
...
今回はファイル名を「upload」としておきました。これでUIパーツ用の「upload.html」ができます。
...
日付、案件、ファイルをサーバー側へ送信できるようにします。

各パーツを表示するためのコードを書きます。案件はシートからデータを取ってくるので後ほどコードを追加します。
...
```
<small>※ inputのtype=dateは一部のブラウザで機能しません。Chromeを利用することを前提で進めています。</small>

モーダルを表示するため「コード.gs」に以下関数を追加。
...
スプシ側にアップロード用のUIが表示できるようになりました。
![アップロード用のUI完成](./images/2021/06/entry475-10.jpg)

### 値がサーバー側で受け取れるかチェック
ここで、値がサーバー側にきちんと行ってるかチェックします。
...
さらに「コード.js」側にも関数を追加し、データの取得を確認します。
...
```
関数「showModal」を実行し、スプレットシート側でファイルや日付を入力し、「ファイルをアップロードする」ボタンをクリックします。
...
日付のデフォルト値を追加します。今日にします。値はゼロパティングで`0000-00-00`年は4桁、月、日は2桁になるように調整します。
...
//省略
```
<small>※ JSのgetMonthメソッドは0始まりなので1プラスする必要があります。</small>
...
//省略
```
取得したい列の最終行を取得し、一気にgetValuesメソットでシートのデータを一気に取得します。
```js
...
getValues取得した値は2次元配列になります。
```js
[[A2],[A3],[A4]]
```

HTML側で値を出力します。EJSちっくな書き方です。
...
## バリデーションで保存するファイルのサイズやタイプを判定
なんでもボンボンアップロードされても困るので、アップロードサイズと種類を制限します。

ファイルがない時もフォーム送信できないようにしておきます。

今回は簡易的にJS側で制御します。

### 初期状態で送信ボタンを押せないようにしておく
ボタンにname属性とdisble属性を付与します。初期状態では`disbled=true`にしてボタンを押せないようにしておきます。
...
`要素.files[0]`でファイル情報が取れます。`type`と`size`を取得します。

ファイルが10Mバイト以下で（1000000バイト）でMIMEタイプがpdf、png、jpgの場合のみボタンをクリックできるようにします。それ以外は警告文を出力し、`value`に格納されたファイル情報も削除します。
...
```
クリック後`didabled=true`に戻した上でサーバーにデータ送信します。
```js
function sendData(e){
  e.disabled = true;
...
### データの取得を確認
GAS側（サーバー側）でデータを受け取れたか確認します。`sendForm`には`console.log`を仕込んだままになっているので、そのまま使います。
...
GASで仕事を効率化して、しっかり本業に集中しましょう！！

続きもありますのでぜひ合わせてお読みください。
...
<div class="article-link__main__title">事例で紹介【GAS】スプシからファイルアップロードし、ドライブに保存する・保存編</div>
