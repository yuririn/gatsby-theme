...
faq:
  - q: "ナビゲーションの文字以外の場所をクリックしても反応しません。"
    a: "それはaタグがインライン要素のままで、コンテンツの幅しか持っていないからです。CSSで **display: block** を指定するか、Flexboxを使用して [aタグ自体のサイズを広げる](#解決法cssでaタグにプロパティdisplayblockをセットするだけ) ことで解決します。"
  ...
  - q: "Flexboxを使っても、aタグ内の文字が上下中央に配置されません。"
    a: "aタグ自身に `display: flex` をかけ、 **align-items: center** を指定しているか確認してください。親のliタグではなく [aタグ側にプロパティをセットする](#解決法aタグにプロパティdisplayflexをセットして配置を調整) のが、当たり判定を広げつつ中央寄せにするコツです。"
memo: メタ情報修正, FAQ追加
---
【2026年3月 メンテナンス済み】  
