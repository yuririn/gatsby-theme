---
title: 【2026年版】CSSでラジオボタン・チェックボックスを爆速カスタマイズ！コピペ用コード付
date: 2020-07-07
...
tags: ["CSS"]
description: 【2026年最新】CSSでラジオボタン・チェックボックスを劇的におしゃれに変える方法を解説。最新のaccent-colorプロパティから、擬似要素を使った高度なカスタマイズまで。コピペで即導入できるコードスニペットを多数掲載し、WordPress（Contact Form 7）やモダンなWeb制作現場ですぐに役立つ内容です。
memo: lightRewrite. 3ヶ月で「ラジオボタン デザイン おしゃれ（7.68位）」。accent-color の解説を追記。display から appearance none に変更。
---
CSSでラジオボタン・チェックボックスの見た目をおしゃれに変える方法を詳しく解説しました。

2026年現在、`accent-color` プロパティの登場で色変更は容易になりましたが、Web制作の現場では「ブランドに合わせた独自デザイン」を求められるシーンが依然として多いです。

この記事では、HTML/CSSを勉強中の方はもちろん、実務でUIデザインをブラッシュアップしたいエンジニアの方に向けて、モダンブラウザに対応した最新のカスタマイズ手法を紹介します。

ラジオボタン・チェックボックスの仕組みを理解すれば、どんなデザインも自由自在に作れるようになります。

*とにかくすぐ使いたい！* という人は、目次から各デザインスニペット（コードサンプル）をコピペして使ってください。

WordPressの定番プラグイン「Contact Form 7」や、モダンなコンポーネント開発でもそのまま使えるコード付きです。（※以前主流だった MW WP Form 等にも対応可能です）
...
![ラジオボタン・チェックボックスはそのままだとまったく可愛くない！](./images/07/entry370-1.png)

2つめ。「各ブラウザごとに用意されているラジオボタン・チェックボックスなどのスタイルが違うから」です。<br>
各ブラウザごとのラジオボタンやチェックボックスを比べてみます（私のマシンがMacなのでIE、Edgeは確認していません）。<br>
比較すると本当にわずかですが細かい違いがわかります。デスクトップだとIEやEdge、モバイルでもっと違うと思います。

![各ブラウザごとに用意されているラジオボタン・チェックボックスなどのスタイルが違うから](./images/07/entry370-2.png)
...
### accent-color である程度色が変えられるようになった【2026年追記】

2021年以降の導入された `accent-color` プロパティを使うと、ラジオボタン・チェックボックスの色が変えられるようになりました。

100% コントロールは出来ませんが、難しいコードを書かなくても多少見た目を調整ができるようになりました。

![ accent-color である程度色が変えられるようになった](./images/07/entry370-7.jpg)

```css
input { accent-color: #ff3b42; }
```

![MDN accent-color のブラウザ対応](./images/07/entry370-6.jpg)

**Safariは注意**
* 「読みやすさ」を優先するため、指定した色が明るすぎると、勝手に少し暗い色に置き換えて表示することがある。
* 他のブラウザでは背景色に合わせてチェックマーク（レ点）を白か黒に自動で切り替えますが、背景色によってはチェックが見えなくなることがあります。

![勝手に少し暗い色に置き換えて表示](./images/07/entry370-8.jpg)

<p><small>Safariは薄ピンクが視認性を考慮して茶色くなった。</small></p>
<p><small>Chromeはaccent-colorはそのまま、チェックやサークルの色が調整される。</small></p>

このように、 **「ブラウザの補正を許さず、100%デザインをコントロールしたい」** という場面では、このあと紹介する「appearance: none を使ったモダンな手法」や「擬似要素を使った完全カスタマイズ」が今でも正解です。
...
```html
<input type="radio" value="multicolor" name="cat" >三毛ねこ</label>
```
### labelタグの中に入れてしまう方法
inputタグをlabelタグの中に入れてしまいます。属性を追加しなくていいので楽チンです。

```html
<label><input type="radio" value="multicolor" name="cat">三毛ねこ</label>
```
...
![基本のラジオボタン・チェックボックスをCSSでカスタマイズ](./images/07/entry370-3.png)

手順としては4ステップ。

1. inputタグを隠す（appearance: none ）
2. labelタグ内にボタンを置くスペースを作る
3. 擬似要素before、afterを使ってラジオボタンの各パーツを作る
4. 擬似クラスを使ってチェックが入った時にスタイルを変える

![基本のラジオボタン・チェックボックスをCSSでカスタマイズ2](./images/07/entry370-4.jpg)

<small>※ 今回はCSS設計などは無視します。</small>

`appearance: none` に指定しますのでinputタグそのものは操作できなくなりますが、labelタグをクリックorタップすればそれに紐づいたinputタグもチェックが入った状態にできます。<br>
...
※ 以前は `display: none` で要素表示を削除してましたが、アクセシビリティに配慮すると `appearance: none` 方が適切です。
...
![基本のラジオボタン・チェックボックスをCSSでカスタマイズ2](./images/07/entry370-4-1.png)
...
![チェックボックスも同じHTMLの構成でスタイリング](./images/07/entry370-5.png)
```css
display: block;
width: 6px;
height: 10px;
