...
faq:
  - q: "Facebook Appの審査が通らなくて心が折れそうです…。"
    a: "個人利用やテスト目的であれば、審査なしの「開発モード」のままでも自分自身のアカウント投稿は取得可能です。2025年現在、**一般公開（ライブモード）にするにはプライバシーポリシーのURL設定などが必須**となるため、[エラー対策のセクション](#突然のエラーデータが取得できない2021-12-24追記)をまずチェックしてください。"
  - q: "トークンの有効期限が切れるたびに再設定が必要ですか？"
    a: "いいえ。初期トークンは1時間で切れますが、手順通りに**「長期アクセストークン」へ交換し、さらに「無期限トークン」を取得**すれば、何度も再設定する必要はなくなります。具体的な手順は[こちらの無期限トークン取得セクション](#無期限アクセストークンに変更)で解説しています。"
  - q: "プログラミング初心者でもInstagramを自分のサイトに埋め込めますか？"
    a: "はい。Facebook側の設定さえクリアできれば、あとは**[2025年版のJSサンプルコード](#jsコードサンプルfetchapiawaitasync)**をコピペして、IDとトークンを書き換えるだけで実装可能です。jQueryを使わない最新の記述（Fetch API）を用意しました。"
---
*【この記事は2025年版にリライト済みです】* 2026年現在も通用するAPI連携の基本テクニックです。最新のブラウザ環境に合わせて内容を再確認しました。
...
![Facebookと連携させる](./images/03/entry448-4.jpg)
