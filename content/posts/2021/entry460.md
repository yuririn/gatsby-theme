...
faq:
  - q: "Slackのカスタムレスポンスで「面白い使い方」はありますか？"
    a: "特定の挨拶に「お疲れ様！」と返したり、社内用語の解説を自動化したりとアイデア次第で無限です。遊び心のあるリアクションを設定することで、チームのコミュニケーションが円滑になります。具体的な [カスタムレスポンスの設定手順](#slackbot返事をお願いカスタムレスポンス) を参考に、独自のbotを作ってみてください。"
  - q: "Slack カスタムレスポンスで改行やメンションは可能ですか？"
    a: "改行は `\\n` （バックスラッシュ + n）を挿入することで可能です。メンション（@ユーザー名）も含めることができますが、カスタムレスポンスの設定画面では少しコツが必要です。詳細は記事内の [カスタムレスポンスの解説](#slackbot返事をお願いカスタムレスポンス) で詳しく解説しています。"
  - q: "Slackで特定の人や期間を指定して「高度な検索」をする方法は？"
    a: "検索窓に `from:@ユーザー名` や `after:2024-01-01` といったフィルターを直接打ち込むか、検索画面のサイドメニューから絞り込めます。「あの資料どこだっけ？」を解決する [最強の検索術](#その1いついつくらいに〇〇さんからもらったpdf資料どこやっけ) をぜひ活用してください。"
