...
faq:
  - q: "セレクトボックスの「選択してください」を選択肢から外したい"
    a: "「選択してください」を表示しつつ選択させないためには、**selected + disabled属性**を付与した上で、CSSの `display: none` で隠すのが正解です。具体的な実装方法は [ケース1の解説](#ケース1セレクトボックス上選択してくださいを表示したいけどその項目を選択肢から外したい) を確認してください。"
  - q: "optgroupを使ってセレクトボックスの項目をグループ化するには？"
    a: "項目が多い場合は **optgroupタグ** を使うことで、ユーザーの可読性が格段に向上します。使い方は簡単で、[こちらのoptgroup実装スニペット](#ケース2optionタグをグルーピングしたい) をコピペするだけでグループ化が可能です。"
  - q: "セレクトボックスで複数選択（multiple）のカスタマイズは可能？"
    a: "ブラウザ依存が非常に強く、CSSのみでリッチにカスタマイズするのは困難です。**アクセシビリティと自由なデザインを両立**させたい場合は、[こちらのチェックボックス（checkbox）代用案](https://ginneko-atelier.com/blogs/entry370/) も検討してみてください。"
---
