...
faq:
  ...
  - q: "画像のプレビューがボケてしまうのですが、解決策はありますか？"
    a: "`getCroppedCanvas` の引数で `width` と `height` を指定することで、出力解像度を調整できます。スマホなどの高画素密度のディスプレイ向けには、表示サイズの2倍程度の数値を指定するのがコツです。"
  - q: "ファイルを選択し直した時にCropperが壊れてしまいます。"
    a: "すでにインスタンスがある場合は、新しいインスタンスを作らずに **`cropper.replace(url)`** メソッドを使いましょう。本記事の[画像の切替処理コード](#cropperjs-でトリム操作できるようにする)で具体的な書き方を解説しています。"
  - q: "大きな画像を読み込むと動作が重くなります。"
    a: "ブラウザのメモリ消費を抑えるため、読み込み時に一度 `canvas` を介して縮小するか、`cropper.js` の `checkOrientation: false` オプションを試すと改善する場合があります。"
---
