...
faq:
  ...
  - q: "目次のハイライトがズレる、または同時に複数点灯してしまいます。"
    a: "それは `threshold`（しきい値）や `rootMargin` の設定が原因かもしれません。特に、見出しの間隔が短い、あるいは見出しが画面上部に来た時に反応させたい場合は、[rootMarginを使って判定基準の線を調整](#基本の使い方)するのが有効です。"
  - q: "古いブラウザへの対応は必要ですか？"
    a: "2026年現在、主要なブラウザはすべてネイティブ対応しています。IEなどの古いブラウザをサポートする必要がある特殊な案件以外は、ポリフィルなしで[そのまま実装](#実際qiita風の目次を作ってみる)して問題ありません。"
---
