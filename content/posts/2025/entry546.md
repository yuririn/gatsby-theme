...
そこで今回は特に人気のフレームワーク3つほど厳選し、使い方を徹底的に比較してみようと思います。
...
CSS の手練れの人が CSS フレームワーク を開発。しかもただで提供していて、そんな4つの問題を解決してくれているんです。そんな人は、フレームワークから始めりゃいいじゃないか！
...
<msg txt="調べてみると Bootstrap 最新のアップデート(v5.5.5)がこの記事を書いた前日でした笑。 X に買収されたあとも、活発に開発しているみたいですね。"></msg>
...
気軽に始める方法としては Bootstrap は CSSとJSのCDNを読み込む、です。
```html:title=HTML
<!DOCTYPE html>
<html lang="ja">
<head>
...
[Bootstrap Documantation](https://getbootstrap.com/docs/5.3/getting-started/introduction/)
...
CSS を HTML のパーツ、たとえば小さなコンポーネントでいうとボタンなどがあるとします。ボタンはたいてい、テキスト（ラベル）、矢印などのアイコン、それらを梱包するラッパーなどで構成されています。
...
Gulp の環境構築なんざムズすぎてインストールできんわい！って人は VS Code のプラグインでSASSのコンパイルもできるし、Prepros（プリプロス）を使えば解決できますよ。
...
コンポーネント（部品）の概念はなく、ひたすらブロックを積み上げるかのようにユーティリティを組み合わせスタイルを構築します。

<msg txt="コンポーネントが主体だとBootstap臭がすごいですよね？ユーティリティクラスを使うことでオリジナリティが欠く欠点をカバーできます。"></msg>

柔軟性が高いということは諸刃の刃。*デザイン力が乏しい方が使うととてつもなくダサくなる* こともあります。

<msg txt="そんな人は、迷わずBootstrapやFoundationを使いましょう。"></msg>


気軽に始める方法として、CDNを読み込む方法です。Tail Wind CSS には JS で動的に CSS を生成する方法と、CSSで静的によみこむ方法があります。
...
たとえば、Tailwin CSS で Bootstrap の primary のボタンを作りたい場合、次のようなコードで再現できます。
...
では、動的に読み込んでみましょう。`@apply` ディレクティブ（ディレクティブとはプログラミングにおける小さな命令みたいなもの）を使えば、カスタムクラスやスタイルを再利用できます。付与するクラスも短くなり、可読性も上がります。
...
エディター VS Code にもいくつか Tailwind CSS 用のコーディングを強力にサポートするプラグインがあります。
...
Foundation の構成要素は9つからなり、Bootstrap（構成要素は6つ） と比べより複雑です。

Foundation の威力を発揮するのは、これらのコンポーネント同士を組み合わせることで実現されます。リッチなウェブサイトを独自に構築したいのであれば、学習コストがどうしてもかかってしまいますが、その分柔軟で強力なデザインができます。
...
実際、コンポーネントを組み合わせてサイトを構築してみるとこんな感じ。
...
こちらは *marketing* のテンプレートに少し手を加えたものです。構築に 3 分もかかりません。
...
Building Blocksとは、FoundationのUIコンポーネントのライブラリを指します。
...
## まとめ・基本は Bootstrap かTailwind CSSを抑えて、個性を出したい場合は Foundation を拡張
今回2025年現在、人気のCSSフレームワークを比較してみました。

個人的には初学者は日本語のリファレンスも豊富な Bootstrap か Tailwind CSS を学んでおけばいいのかなぁという感想です。
...
フレームワークを利用するにせよ、CSSの基本の「キ」が分かってないと起こる問題があります。表面だけでフレームワークを使ってるから大丈夫と過信すると思わぬ表示崩れが発生していることもあります。実際にそういうコードに遭遇しました。

すでに調整されたフレームワークだからと、過信せずに基本のブラウザごとのデバックを怠ってはいけません。そんな思いもあり、この記事を書きました。
