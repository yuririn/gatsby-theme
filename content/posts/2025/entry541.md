...
この設定をした理由は、最小単位（Module）でデバッグを行いたかったからです。そのほか、ESモジュールをトランスパイル（CommonJSをソースコードへ変換）したことも起因しています。

もう時代的にはこの設定は必要ないのでまたこの記事も後日見直します。
...
copilot は Github を所有する Microsoft が提供するサービスです。プログラミングでは老舗なだけでなく、コンプラガン無視すると、あれだけ大量のコードのあるGithubの相当優秀なはず笑。

そのままコードを投げて、テスト内容で意図しない返答が返ってきたらそもそも自分が~~クソ~~コードを書いている可能性を疑いましょう。
...
たとえば、Instagram 投稿一覧の取得などをAPIで呼び出すようなプログラムの場合、まずはAPIの呼び出しに成功したかどうかを確認する必要がありますよね？

そんな感じでひとつづつ細分化してテストコードのプロンプトをちょっとづつ進めます。
...
copilot を使いながらテストをしていて、感動したことが多々あろ、ありました。
