...
const Thanks = () => {
	return (
		<div>
			...
			<p className="u-mblg">もしくは、FBページからお問い合わせいただくか、TwitterアカウントにDMをお願いします。</p>
			<ul className="u-mblg">
...
const Error = ({ name, email, agreement }) => {
	if (name === '' && email === '' && agreement === '') {
		...
		return < p className={ContactStyles.error} >※ 必須項目に入力が漏れているか、入力に誤りがある可能性があります。</p >
	}
}
const Detail = ({ name }) => {
	let description = ''
						...
						<dd>
							{
								types.map((value) => {
									return (
										<label><input type="radio" name="type" value={value.name} onChange={onChange} /><span>{value.name}</span></label>
									)
								})
