...
const Affili = ({size}) => {
  ...
  // const categories = siteMetadata.category;
  const arry = AffArry.filter(item => {
  ...
  if(arry.length > 0){
    return <AffliWrapper dangerouslySetInnerHTML={{__html:arry[random].code}}/>
...
export default Affili;

const AffliWrapper = styled.div`
  text-align:center;
  margin: 30px auto;
`
