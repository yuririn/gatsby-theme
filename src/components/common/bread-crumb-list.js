...
const BreadCrumbList = ({ type, current = '', cate = '', tag = '' }) => {
  ...
  return (
    <BreadCrumb className={thisClass}>
      {type === `ad` || type === `ad-tag` ? (
          <li><Link to="/choco-blog/">{ad.title}</Link></li>
        ) : (
          <li><Link to="/">銀ねこアトリエ</Link></li>
        )}

        {type === `blog` && (
          <li><Link to="/blogs/">ノマドブログ</Link></li>
        )}
        {cate !== '' && (
          <li><Link to={cate.url}>{cate.name}</Link></li>
        )}
        {tag !== '' && (
          <li><Link to={`/blogs/tags/${tag}`}>{tag}</Link></li>
        )}
